...
class SpmGlm:
    ...
    def get_ciftis(self, mask=(None, None), TR=1000, extra=None):
        """
        ...
        for m, s in zip(mask, struct):
            if m is not None:
                ...
                # get raw data
                raw_tmp = nt.sample_images(self.rawdata_files, coords)
                data['raw'].append(raw_tmp)

                # rerun glm
                beta_tmp, info, y_filt_tmp, y_hat_tmp, y_adj_tmp, residuals_tmp = self.rerun_glm(raw_tmp)
        ...
        img_raw = cifti2.Cifti2Image(dataobj=raw, header=header, extra=extra)
        img_filt = cifti2.Cifti2Image(dataobj=y_filt, header=header, extra=extra)
        img_hat = cifti2.Cifti2Image(dataobj=y_hat, header=header, extra=extra)
        img_adj = cifti2.Cifti2Image(dataobj=y_adj, header=header, extra=extra)
        img_res = cifti2.Cifti2Image(dataobj=residuals, header=header, extra=extra)
